...
class Model(private var source: TextureSource) {
    ...
    fun getTextures(lights: Map<Block.Side, Int>): Array<ModelTexture>{
        ...
        elements.forEach { cube ->
            cube.faces.keys.forEach { faceKey ->
                ...
                val texture = ModelTexture(clearResourcePath(imagePath), source, light, points[0].clone(), points[1].clone(), points[2].clone(), points[3].clone(), subTexture = face.uv)
                if(!cube.shade)
                    texture.light = 15
                texture.cullFace(face.cullFace)

                if(cube.scale.x != 1.0)
                    texture.localTransform(cube.scale)
                if(cube.rotation.angle != 0.0)
                    texture.localTransform(cube.rotation)
    ...
    private class TexturePoint(x: Double, y: Double, z: Double): Point3D(x / 16.0, y / 16.0, z / 16.0)
